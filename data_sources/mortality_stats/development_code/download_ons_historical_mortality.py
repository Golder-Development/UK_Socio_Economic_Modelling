    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            try:
                ...
                logger.info(f"    • {sheet_name}: {df.shape[0]} rows × {df.shape[1]} cols")
...
def extract_yearly_totals(sheets_data):
    ...
    for sheet_name, df in sheets_data.items():
        ...
        if any(keyword in sheet_name.lower() for keyword in ['total', 'summary', 'annual', 'year']):
            ...
            if year_col and deaths_col:
                ...
                result['total_deaths'] = pd.to_numeric(result['total_deaths'], errors='coerce')
...
def main():
        ...
        if downloaded:
            ...
            if extension in ['.xlsx', '.xls']:
                ...
                if yearly_data is not None:
                    ...
                    logger.info(f"  Year range: {yearly_data['year'].min():.0f} - {yearly_data['year'].max():.0f}")
    ...
    if all_yearly_data:
        ...
        combined = combined.groupby('year', as_index=False)['total_deaths'].max()  # Take max to avoid duplicates
        ...
        logger.info(f"✓ Range: {combined['year'].min():.0f} - {combined['year'].max():.0f}")
        logger.info(f"\nData preview:")
        print(combined.to_string(index=False))
    else:
        logger.warning("\nNo yearly data extracted. Files may need manual inspection.")
        logger.info(f"\nDownloaded files are in: {DOWNLOADS_DIR}")
        logger.info("Please review them manually to identify the correct sheets/columns.")
