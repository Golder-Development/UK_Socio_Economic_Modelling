...
def extract_deaths_by_cause(df, year_col="YR", icd_col="ICD-10", deaths_col="NDTHS"):
    ...
    # Filter valid
    df_valid = df[
        df["year"].notna() & df["deaths"].notna() & (df["icd10"] != "")
    ].copy()
    ...
    by_cause = df_valid.groupby(["year", "icd10_chapter"], as_index=False)[
        "deaths"
    ].sum()
...
def main():
    ...
    if df_2001_2019 is not None:
        ...
        if yearly is not None:
            logger.info(
                f"✓ Extracted {len(yearly)} years: {yearly['year'].min()}-{yearly['year'].max()}"
            )
    ...
    if df_api is not None:
        logger.info(
            f"✓ Loaded {len(df_api)} years from API: {df_api['year'].min():.0f}-{df_api['year'].max():.0f}"
        )
    ...
    if all_yearly_totals:
        ...
        combined_yearly = combined_yearly.groupby("year", as_index=False)[
            "total_deaths"
        ].max()
        ...
        logger.info(
            f"✓ Range: {combined_yearly['year'].min():.0f} - {combined_yearly['year'].max():.0f}"
        )
        logger.info(f"\n📊 Yearly totals:")
    ...
    if all_cause_data:
        ...
        combined_cause = combined_cause.groupby(
            ["year", "icd10_chapter"], as_index=False
        )["total_deaths"].sum()

        # Add chapter names
        chapter_names = get_icd10_chapter_names()
        combined_cause["cause_category"] = combined_cause["icd10_chapter"].map(
            chapter_names
        )
        ...
        sample = combined_cause[combined_cause["year"] == 2019].nlargest(
            10, "total_deaths"
        )
        print(
            sample[
                ["year", "icd10_chapter", "cause_category", "total_deaths"]
