    ...
    for filename, sheet_name in files:
        ...
        try:
            ...
            if not df.empty:
                ...
                df["deaths"] = pd.to_numeric(df["deaths"], errors="coerce").astype(
                    "Int64"
                )
                ...
                logger.info(
                    f"  ✓ {len(df):,} rows ({df['year'].min():.0f}-{df['year'].max():.0f})"
                )
                all_data.append(df)
            else:
...
def load_compiled_2001_2019():
    ...
    try:
        ...
        df["year"] = pd.to_numeric(
            df.get("yr", df.get("year")), errors="coerce"
        ).astype("Int64")
        ...
        death_cols = [c for c in df.columns if "ndth" in c or "death" in c]
        if death_cols:
            df["deaths"] = pd.to_numeric(df[death_cols[0]], errors="coerce").astype(
                "Int64"
            )
        else:
            df["deaths"] = 0
...
def main():
    ...
    summary = all_data.groupby(
        ["year", "cause", "sex", "age"], as_index=False, dropna=False
    )["deaths"].sum()
    ...
    logger.info(
        f"✓ Saved comprehensive: {out_comprehensive.name} ({len(summary):,} records)"
    )
    ...
    logger.info(
        f"  Min year: {yearly.loc[yearly['total_deaths'].idxmin(), 'year']:.0f} ({yearly['total_deaths'].min():,.0f} deaths)"
    )
    logger.info(
        f"  Max year: {yearly.loc[yearly['total_deaths'].idxmax(), 'year']:.0f} ({yearly['total_deaths'].max():,.0f} deaths)"
    )

    logger.info(f"\nDimensions in comprehensive dataset:")
    logger.info(
        f"  Years: {summary['year'].min():.0f} - {summary['year'].max():.0f} ({summary['year'].nunique()} distinct)"
    )
