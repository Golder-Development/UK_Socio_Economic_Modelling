...
def load_data():
    ...
    logger.info(
        f"  ✓ {len(population):,} records ({population['YR'].min()}-{population['YR'].max()})"
    )
...
def prepare_data(mortality, population):
    ...
    mort_std["sex"] = (
        mort_std["sex"].map({"Male": 1, "Female": 2}).fillna(mort_std["sex"])
    )
    ...
    missing_years = [
        y
        for y in range(int(mort_years[0]), int(mort_years[-1]) + 1)
        if y not in mort_years
    ]
    if missing_years:
...
def aggregate_by_dimensions(mort_std, pop_std):
    ...
    mort_agg = mort_std.groupby(
        ["YR", "cause", "sex", "age"], as_index=False, dropna=False
    )["deaths"].sum()
...
def calculate_mortality_rates(mort_agg, pop_agg):
    ...
    merged = mort_agg.merge(pop_agg, on=["YR", "sex", "age"], how="left")
    ...
    merged_with_rates["sex"] = (
        merged_with_rates["sex"].map(sex_map).fillna(merged_with_rates["sex"])
    )
...
def calculate_yearly_totals(rates):
    ...
    yearly["mortality_rate_per_100k"] = (
        yearly["deaths"] / yearly["population"]
    ) * 100000
