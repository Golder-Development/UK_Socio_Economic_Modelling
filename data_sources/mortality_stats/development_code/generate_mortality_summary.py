...
def prepare_data(mortality, population):
    ...
    mort['deaths'] = pd.to_numeric(mort['deaths'], errors='coerce').fillna(0).astype(int)
...
def calculate_summary(mortality, population_raw):
    ...
    drug_deaths = mort_all[mort_all['is_drug']].groupby('YR')['deaths'].sum().reset_index()
    ...
    summary['total_deaths_per_100k'] = (summary['total_deaths'] / summary['total_population']) * 100000
    summary['drug_deaths_per_100k'] = (summary['drug_deaths'] / summary['total_population']) * 100000
    
    # Select final columns
    summary = summary[[
        'year', 'total_deaths', 'total_population', 'total_deaths_per_100k',
        'drug_deaths', 'drug_deaths_per_100k'
    ]]
    ...
    logger.info(f"  Year range: {summary['year'].min():.0f}-{summary['year'].max():.0f}")
...
def main():
    ...
    logger.info(f"  Change: {summary.iloc[-1]['total_deaths_per_100k'] - summary.iloc[0]['total_deaths_per_100k']:.2f} ({((summary.iloc[-1]['total_deaths_per_100k'] / summary.iloc[0]['total_deaths_per_100k']) - 1) * 100:.1f}%)")
    ...
    if len(drug_data) > 0:
        ...
        logger.info(f"  {drug_data['year'].max():.0f}: {drug_data.iloc[-1]['drug_deaths_per_100k']:.3f} per 100,000")
        logger.info(f"  Peak: {drug_data['drug_deaths_per_100k'].max():.3f} per 100,000 ({int(drug_data.loc[drug_data['drug_deaths_per_100k'].idxmax(), 'year'])})")
    else:
        logger.info("  No drug-related deaths recorded (data not available for pre-2001 years)")
