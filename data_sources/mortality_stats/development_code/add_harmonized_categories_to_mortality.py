...
def add_harmonized_categories_to_file(input_file, output_file, harmonized_mapping):
    ...
    df = df.merge(
        harmonized_lookup[['code', 'icd_version', 'harmonized_category', 
                          'harmonized_category_name', 'classification_confidence']], 
    ...
    harm_cols = ['harmonized_category', 'harmonized_category_name', 'classification_confidence']
    ...
    logger.info(f"  Matched harmonized categories: {matched:,} / {total:,} ({match_rate:.1f}%)")
    
    # Show category distribution
    if 'harmonized_category_name' in df.columns:
        logger.info(f"\n  Distribution of deaths by harmonized category:")
        cat_summary = df.groupby('harmonized_category_name')['deaths'].sum().sort_values(ascending=False)
...
def main():
    ...
    logger.info(f"Categories: {harmonized_mapping['harmonized_category_name'].nunique()}")
