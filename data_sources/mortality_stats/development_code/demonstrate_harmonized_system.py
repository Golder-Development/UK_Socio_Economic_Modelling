...
print("=" * 80)
sample = df[df['year'] == 1901][['year', 'cause', 'cause_description', 
                                  'harmonized_category_name', 'sex', 'age', 'deaths']].head(10)
...
yearly_by_category = df.groupby(['year', 'harmonized_category_name'])['deaths'].sum().reset_index()

# Get top categories by total deaths
top_categories = df.groupby('harmonized_category_name')['deaths'].sum().nlargest(5).index.tolist()
...
top_cat_data = yearly_by_category[yearly_by_category['harmonized_category_name'] == top_categories[0]]
...
age_dist = df_1901.groupby(['harmonized_category_name', 'age'])['deaths'].sum().reset_index()

for cat in top_categories[:3]:
    cat_age = age_dist[age_dist['harmonized_category_name'] == cat].nlargest(5, 'deaths')
    print(f"\n{cat}:")
...
infectious_codes = mapping_df[mapping_df['harmonized_category_name'] == 'Infectious and Parasitic Diseases']
...
for icd_version in ['ICD-1 (1901-1910)', 'ICD-5 (1940-1949)', 'ICD-9c (1994-2000)']:
    version_codes = infectious_codes[infectious_codes['icd_version'] == icd_version].head(3)
    print(f"\n{icd_version}:")
...
print(f"Rows with harmonized categories: {df['harmonized_category_name'].notna().sum():,} / {len(df):,}")
