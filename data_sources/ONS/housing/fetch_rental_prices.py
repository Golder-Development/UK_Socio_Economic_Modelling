"""
Script to fetch Index of Private Housing Rental Prices from the ONS API.

The Index of Private Housing Rental Prices (IPHRP) tracks the prices paid for 
renting property in the private rental sector in the UK.
"""
import requests
import pandas as pd
import logging
...
DATASET_NAME = "Index of Private Housing Rental Prices"
...
def get_list_of_datasets():
    ...
    try:
        while True:
            r = requests.get(ROOT_URL + "datasets", params={"offset": offset, "limit": limit})
            r.raise_for_status()
            results = r.json()
...
def main():
    ...
    for dim_name, options_dict in valid_dimensions.items():
        if dim_name.lower() in ["time", "date", "period"]:
            api_dimensions[dim_name] = "*"
        elif dim_name.lower() in ["geography", "location", "area"]:
            # Try to get UK aggregate
            uk_options = [k for k, v in options_dict.items() if "uk" in v.lower() or "kingdom" in v.lower()]
            if uk_options:
                api_dimensions[dim_name] = uk_options[0]
                logger.info(f"  Using UK aggregate for {dim_name}: {uk_options[0]}")
            else:
    ...
    output_file = extract_dir / "rental_prices_data.csv"
