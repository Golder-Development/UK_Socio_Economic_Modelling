...
def get_dataset_by_name(datasets, target_name):
    ...
    Returns the first dataset object whose name
    contains the given target_name string.
...
def get_dimensions(edition_url):
    """Builds dictionary of all valid options for
    all dimensions of a given dataset,
    with descriptions for each option.
    Individual obvserviations can later be obtained
    by choosing from these options.
    Ranges can later be obtained by replacing
    one dimesion with the wildcard '*'.
    ...
    for dimension in results.get("items"):
        ...
        sresults = sr.json()
        # TODO! Could add in paging here, as there
        # *could* be multiple pages of valid options.
        logger.info(f"\tHas {sresults.get('count')} options")
        # valid_options = [item.get("option") for
        # item in sresults.get("items")]
        option_descriptions = {
...
def choose_dimensions(valid_dims, overrides={}):
    """For each dimension, choose a single valid option
    (except for 'time', where
    ...
    """
    # By default, choose first valid item for all
    # dimensions; then override where needed:
    chosen_dimensions = {
        k: next(iter(v.keys())) for k, v in valid_dims.items()
    }
...
def get_observations(edition_url, dimensions):
    ...
    for observation in results.get("observations"):
        ...
        obs_dict = {"id": id, "observation": observation.get("observation")}
        summary.append(obs_dict)
...
def demo():
    ...
    # Repeat the process for a second time series, GDP
